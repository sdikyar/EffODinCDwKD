...
@PIPELINES.register_module()
class LoadTeachImageFromFolder(LoadImageFromFile):
    """
    Loads a Teacher image and STRICTLY RESIZES it to match the already loaded RGB image.
    """
    def __init__(self, folder_path, key='t_img', **kwargs):
    ...
    def __call__(self, results):
        ...
        original_filename = results['img_info']['filename']
        filename = osp.join(self.folder_path, osp.basename(original_filename))

        # 1. Load the Teacher Image
        try:
            img_bytes = self.file_client.get(filename)
            img = mmcv.imfrombytes(img_bytes, flag=self.color_type, channel_order=self.channel_order)
        except Exception as e:
            print(f"[ERROR] Failed to load Teacher Image: {filename}")
            raise e

        # 2. FORCE RESIZE to Match RGB (The Fix)
        # We look at the RGB image that was just loaded (stored in results['img'])
        if 'img' in results:
            rgb_h, rgb_w = results['img'].shape[:2]
            pd_h, pd_w = img.shape[:2]

            # Only resize if dimensions differ
            if rgb_h != pd_h or rgb_w != pd_w:
                # print(f"[Info] Resizing PD {img.shape} -> RGB {(rgb_h, rgb_w)}")
                # return_scale=False ensures we get just the image back
                img = mmcv.imresize(img, (rgb_w, rgb_h), return_scale=False)
        ...
        if self.key not in results.get('img_fields', []):
            results['img_fields'].append(self.key)

...
class DetectionDistiller_Retina_GF_LOG(BaseDetector):
    ...
    def __init__(self,
        ...
        super(DetectionDistiller_Retina_GF_LOG, self).__init__()
    ...
    def forward_train(self, 
        ...
        with torch.no_grad():
            ...
            # tea_cls_scores, tea_reg_deltas, tea_centernesses = self.teacher.bbox_head(fea_t)
            tea_cls_scores, tea_reg_deltas = self.teacher.bbox_head(fea_t)
        fea_s = self.student.extract_feat(img)
        # stu_cls_scores, stu_reg_deltas, tea_centernesses = self.student.bbox_head(fea_s)
        stu_cls_scores, stu_reg_deltas = self.student.bbox_head(fea_s)
        
        for i, item_loc in enumerate(self.distill_cfg):
            # Identify the module type based on the config string
            is_fpn = 'neck' in item_loc.student_module
            is_head = 'bbox_head' in item_loc.student_module

            for item_loss in item_loc.methods:
                loss_name = item_loss.name
                
                # Calculate all losses (the module will return None for disabled parts)
                res = self.distill_losses[loss_name](
                        fea_s[i], fea_t[i], 
                        stu_cls_scores[i], tea_cls_scores[i], 
                        stu_reg_deltas[i], tea_reg_deltas[i], 
                        img_metas, kwargs["gt_bboxes"])
                
                # --- CASE 1: FPN NECK (Feature Distillation) ---
                if is_fpn:
                    # res[0] = Global Loss
                    if res[0] is not None:
                        student_loss[f"{loss_name}_global"] = res[0]
                    
                    # res[1] = Fg/Bg Tuple (fg, bg)
                    if res[1] is not None:
                        student_loss[f"{loss_name}_fg"] = res[1][0]
                        student_loss[f"{loss_name}_bg"] = res[1][1]

                    # res[2] = Instance Loss (if you enabled it)
                    if res[2] is not None:
                         student_loss[f"{loss_name}_instances"] = res[2]

                # --- CASE 2: BBOX HEAD (Logits/Response Distillation) ---
                elif is_head:
                    # res[3] = Logits Loss

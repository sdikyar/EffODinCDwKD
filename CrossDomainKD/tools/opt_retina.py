...
import warnings
import os
from writing_config_retina import over_write
...
def get_best_value(opt):
    args = parse_args()
    over_write(opt['global_trans1'], opt['global_trans2'], opt['global_trans3'], args.config)
    bbox_mAP = main(args)
    return bbox_mAP


def objective(trial):
    global_trans1 = trial.suggest_categorical('global_trans1',
                                            ['no','satt','catt','mask'])
    global_trans2 = trial.suggest_categorical('global_trans2',
                                            ['no','scale_r1','scale_r2','multi_scale_r4','local_s1','local_s2','local_s4','scale'])
    global_trans3 = trial.suggest_categorical('global_trans3',
    ...
    opt = dict()
    opt['global_trans1'] = global_trans1
    opt['global_trans2'] = global_trans2
    opt['global_trans3'] = global_trans3
...
if __name__ == "__main__":
    ...
    # Append the timeout directly to the URL string. No special import needed.
    storage_name_with_timeout = f"sqlite:///{db_filename}?timeout=60"

    study = optuna.create_study(
        study_name=study_name,
        storage=storage_name_with_timeout, # Pass the URL string directly
        direction='maximize',
